...
begin
  ...
  xSaude[xContador]:= inputbox('Saúde','Digite seu estado de saúde, B=Boa ou R=Ruim','');

  if xSexo[xContador] = 'F' then
    ShowMessage('Vocé é inapto(a) ao serviço militar')
    else if (xSexo[xContador] = 'M') and (xIdade[xContador] >= 18)
     and (xSaude[xContador] = 'B') then
      ShowMessage('Vocé está apto ao serviço militar')
      else ShowMessage('Vocé está inapto ao serviço militar');
  ...
  ShowMessage('Há '+IntToStr(xApto) +' pessoa(s) apta(s) e ' +IntToStr(xInapto)
  +' pessoa(s) inapta(s) ao serviço militar')
